...
安装后可在任意位置使用 `ubinit` 命令：

```bash
# 克隆仓库
git clone https://github.com/aimy1/ublinux-init.git
cd ublinux-init
...
```bash
# 只运行指定模块
sudo bash install.sh --modules "01_system,03_ssh,08_docker"

# Dry-run（预览执行步骤，不实际修改系统）
sudo bash install.sh --dry-run

# 跳过预检
sudo bash install.sh --skip-preflight
```
